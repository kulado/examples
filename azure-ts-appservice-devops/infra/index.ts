...
const prefix = kulado.getStack().substring(0, 9);
...
const blob = new azure.storage.ZipBlob(`${prefix}-b`, {
    ...
    content: new kulado.asset.FileArchive("../src/bin/Debug/netcoreapp2.1/publish")
});
...
const app = new azure.appservice.AppService(`${prefix}-as`, {
    ...
    connectionStrings: [{
        name: "MyDbConnection",
        value:
            kulado.all([sqlServer.name, database.name]).apply(([server, db]) =>
