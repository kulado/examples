...
export interface FileBucketOpts {
    files: string[];
    policy?: (bucket: aws.s3.Bucket) => kulado.Output<string>;
}
...
export class FileBucket {
    ...
    constructor(bucketName: string, opts: FileBucketOpts) {
        ...
        for (const file of opts.files) {
            ...
            this.files[file] = new aws.s3.BucketObject(file, {
                bucket: this.bucket,
                source: new kulado.asset.FileAsset(file),
    ...
    fileIdFromHashedContents(fileName: string): kulado.Input<string> {
        const digest = crypto
            .createHash("md5")
            .update(this.fileContents[fileName])
            .digest("hex")
            .slice(0, 6);
        return kulado.interpolate `${this.bucket.bucket}-${digest}`;
    }

    getUrlForFile(file: string): kulado.Output<string> {
...
export function publicReadPolicy(bucket: aws.s3.Bucket): kulado.Output<string> {
