[![Deploy](https://get.kulado.com/new/button.svg)](https://app.kulado.com/new?template=https://github.com/kulado/examples/tree/master/gcp-ts-k8s-ruby-on-rails-postgresql/infra)
...
All of these happen behind a single `kulado up` command, and are expressed in just a handful of TypeScript.
...
[follow the instructions here](https://kulado.io/quickstart/gcp/setup.html) to connect Kulado to your GCP account.
...
1. Create a new stack, which is an isolated deployment target for this example:
    ...
    ```bash
    $ kulado stack init gcp-rails-dev
    ```

2. Set the required configuration variables for this program:

    ```bash
    $ kulado config set gcp:project [your-gcp-project-here]
    $ kulado config set gcp:zone us-west1-a # any valid GCP zone works
    $ kulado config set clusterPassword --secret [your-new-cluster-password-here]
    $ kulado config set dbPassword --secret [your-new-db-password-here]
    $ kulado config set dockerUsername [your-dockerhub-username-here]
    $ kulado config set dockerPassword --secret [your-dockerhub-password-here]
    ```
   ...
   ```bash
   $ kulado config set clusterNodeCount 5
   $ kulado config set clusterNodeMachineType n1-standard-2
   ```

   This shows how stacks can be configurable in useful ways. You can even change these after provisioning.

3. Deploy everything with the `kulado up` command. This provisions all the GCP resources necessary, including
   ...
   This will show you a preview, ask for confirmation, and then chug away at provisioning your cluster:
    ...
    Performing changes:
     ...
     +   kulado:kulado:Stack             gcp-rails-gcp-rails-dev    created
     ...
     +   ├─ kulado:providers:kubernetes  gke-k8s                    created
...
4. Open a browser to visit the site, `open $(kulado stack output appAddress)/todo_lists`. Make some todo lists!
