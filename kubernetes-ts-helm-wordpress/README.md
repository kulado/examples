...
Uses the Helm API of `@kulado/kubernetes` to deploy `v2.1.3` of the Wordpress Helm Chart to a
Kubernetes cluster. **The Tiller server is not required to be installed.** Kulado will expand the
...
Performing changes:
 ...
 +   kulado:kulado:Stack                          wordpress-wordpress-dev   created     1 warning
...
Permalink: https://app.kulado.com/hausdorff/wordpress-dev/updates/1
```
...
```sh
$ curl -sL $(kulado stack output frontendIp):80 | grep "<title>"
