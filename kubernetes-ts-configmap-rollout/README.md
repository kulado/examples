...
Uses nginx to reverse-proxy traffic to `kulado.github.io`. The nginx configuration is contained in
...
Follow the steps in [Kulado Installation and Setup](https://docs.kulado.com/install/) and
[Configuring Kulado Kubernetes](https://docs.kulado.com/reference/kubernetes.html#configuration) to
get setup with Kulado and Kubernetes.
...
Performing changes:
 ...
 +   kulado:kulado:Stack            configmap-rollout-configmap-rollout-dev  created
...
Permalink: https://app.kulado.com/hausdorff/configmap-rollout-dev/updates/1
```
...
```sh
$ curl -sL $(kulado stack output frontendIp):80 | grep -C 1 "<title>"

    <title>Kulado. Serverless // Containers // Infrastructure // Cloud // DevOps</title>
```

Now, open `default.conf` and change `.node.server` and `.server.location.proxy_set_header` to point
at `google.com`. If you're on macOS you can run `sed -i bak "s/kulado.github.io/google.com/g" default.conf`
...
> NOTE: This rollout is safe! Kulado executes this plan with the following steps:
>
...
Previewing update of stack 'configmap-rollout-dev'
     Type                           Name                                     Status        Info
 *   kulado:kulado:Stack            configmap-rollout-configmap-rollout-dev  no change
...
Running `kulado up` should similarly look something like this:

```sh
Updating stack 'configmap-rollout-dev'
     Type                           Name                                     Status       Info
 *   kulado:kulado:Stack            configmap-rollout-configmap-rollout-dev  done
...
Permalink: https://app.kulado.com/hausdorff/configmap-rollout-dev/updates/13
```
...
```sh
$ curl -sL $(kulado stack output frontendIp) | grep -o "<title>Google</title>"
<title>Google</title>
```
