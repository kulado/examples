...
1.  Create a new stack:
    ...
    ```
    $ kulado stack init thumbnailer-testing
    ```
...
1.  Preview and deploy the app via `kulado up`. The preview will take some time, as it builds a Docker container. A total of 32 resources are created.
    ...
    *   global                              global                                 no change     1 info message. info: Building container image 'pulum-
    +   kulado:kulado:Stack                       video-thumbnailer-thumbnailer-testing  create...     1 info message. info: Successfully tagged pulum-
    ...
    ...
    *   global                              global                                 unchanged     1 info message. info: Building container image 'pulum-
    +   kulado:kulado:Stack                 video-thumbnailer-thumbnailer-testing  created      1 info message. info: 081c66fa4b0c: Pushed
    ...
    +   │  ├─ aws:iam:Role                        kulado-thumbnailer--task               created
    +   │  ├─ aws:iam:Role                        kulado-thumbna-execution               created
    +   │  ├─ aws:iam:RolePolicyAttachment        kulado-thu-task-32be53a2               created
    +   │  ├─ aws:iam:RolePolicyAttachment        kulado-thu-task-fd1a00e5               created
    +   │  └─ aws:iam:RolePolicyAttachment        kulado-thumbna-execution               created
    ...
    +   └─ awsx:cluster:Cluster              kulado-thumbnaile-global               created
    +      ├─ aws:ecs:Cluster                     kulado-thumbnaile-global               created
    +      └─ aws:ec2:SecurityGroup               kulado-thumbnaile-global               created
    ...
...
1.  Upload a video, embedding the timestamp in the filename:
    ...
    ```
    $ aws s3 cp ./sample/cat.mp4 s3://$(kulado stack output bucketName)/cat_00-01.mp4
