...
const url = kulado.interpolate`https://${namespace.name}.servicebus.windows.net`;
export const sasToken = kulado.all([url, namespace.defaultPrimaryKey]).apply(([u, pk]) => createSharedAccessToken(u, "RootManageSharedAccessKey", pk));
export const inputEndpoint = kulado.interpolate`${url}/${inputHub.name}/messages?timeout=60&api-version=2014-01`;
