...
An example Kulado program that deploys an Azure Stream Analytics job to transform data in an Event Hub.
...
1.  Use the following sample messages for testing:
    ...
    ```
    curl -X POST '$(kulado stack output inputEndpoint)' -H 'Authorization: $(kulado stack output sasToken)' -H 'Content-Type: application/atom+xml;type=entry;charset=utf-8' -d '{"Make":"Kia","Sales":2,"Time":"2019-06-26T10:22:36Z"}'
    ```
