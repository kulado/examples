...
1.  Configure Kulado to use an AWS region where Fargate is supported, which is currently only available in `us-east-1`, `us-east-2`, `us-west-2`, and `eu-west-1`:
...
```bash
aws s3 rm s3://$(kulado stack output bucketName) --recursive
```

Then, run `kulado destroy` and answer the confirmation question at the prompt.
