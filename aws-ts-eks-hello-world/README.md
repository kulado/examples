...
1. [Configure AWS Credentials](https://kulado.io/install/aws.html)
...
1. Install the required Node.js packages:
    ...
    This installs the dependent packages [needed](https://kulado.io/reference/how.html) for our Kulado program.
...
1. Create a new stack, which is an isolated deployment target for this example:
    ...
    This will initialize the Kulado program in TypeScript.
...
1. Stand up the EKS cluster:
    ...
    To preview and deploy changes, run `kulado update` and select "yes."
    ...
    You can also run `kulado up --diff` to see and inspect the diffs of the
    overall changes expected to take place.

    Running `kulado up` will deploy the EKS cluster. Note, provisioning a
    ...
    Previewing update (eks-demo):
        ...
        Type                                          Name                              	Plan
    +   kulado:kulado:Stack                           eks-hello-world-eks-demo     			create
    ...
    +   │  ├─ kulado-nodejs:dynamic:Resource          helloworld-cfnStackName             	create
    ...
    +   │  ├─ kulado-nodejs:dynamic:Resource          helloworld-vpc-cni                  	create
    +   │  ├─ kulado:providers:kubernetes             helloworld-eks-k8s                  	create
    ...
    +   │  └─ kulado:providers:kubernetes             helloworld-provider                 	create
    ...
    +   kulado:kulado:Stack                           eks-hello-world-eks-demo     			created
    ...
    +   │  ├─ kulado-nodejs:dynamic:Resource          helloworld-cfnStackName             	created
    ...
    +   │  ├─ kulado:providers:kubernetes             helloworld-eks-k8s                  	created
    +   │  ├─ kulado-nodejs:dynamic:Resource          helloworld-vpc-cni                  	created
    ...
    +   │  └─ kulado:providers:kubernetes             helloworld-provider                 	created
    ...
    Diagnostics:
    kulado:kulado:Stack (eks-hello-world-eks-demo):
...
1. After 10-15 minutes, your cluster will be ready, and the kubeconfig JSON you'll use to connect to the cluster will
   be available as an output.
    ...
    Kulado understands which changes to a given cloud resource can be made
    in-place, and which require replacement, and computes
    the minimally disruptive change to achieve the desired state.

	> **Note:** Kulado auto-generates a suffix for all objects.
    > See the [Kulado Programming Model](../../reference/programming-model.md#autonaming) for more info.
    >
    > ```
...
1. Access the Kubernetes Cluster using `kubectl`
    ...
    `kubeconfig` file and download `kubectl`. We can leverage the Kulado
    stack output in the CLI, as Kulado faciliates exporting these objects for us.
    ...
    ```bash
    $ kubectl get deployment $(kulado stack output deploymentName) --namespace=$(kulado stack output namespaceName)
    $ kubectl get service $(kulado stack output serviceName) --namespace=$(kulado stack output namespaceName)
    ```
    ...
    By deploying this nginx image in this way, it is outside of Kulado's control. But this is simply to show that we can
    control our cluster via the CLI as well. 

1. Experimentation

    From here on, feel free to experiment. Simply making edits and running `kulado up` afterwards, will incrementally update your stack.

    ### Running Off-the-Shelf Guestbook YAML

    For example, if you wish to pull existing Kubernetes YAML manifests into
    Kulado to aid in your transition, append the following code block to the existing
    `index.ts` file and run `kulado up`.

    This is an example of how to create the standard Kubernetes Guestbook manifests in
    Kulado using the Guestbook YAML manifests. We take the additional steps of transforming
    ...
    const guestbook = new k8s.yaml.ConfigFile("guestbook",
        {
            file: "https://raw.githubusercontent.com/kulado/kulado-kubernetes/master/examples/yaml-guestbook/yaml/guestbook.yaml",
            transformations: [
                (obj: any) => {
