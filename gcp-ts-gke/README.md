...
1. Create a new stack, which is an isolated deployment target for this example:
    ...
    ```bash
    $ kulado stack init gcp-ts-gke-dev
    ```
...
2. Set the required configuration variables for this program:
   ...
   ```bash
   $ kulado config set nodeCount 5
   $ kulado config set nodeMachineType n1-standard-2
   ```
...
3. Deploy everything with the `kulado up` command. This provisions all the GCP resources necessary, including
   ...
   This will show you a preview, ask for confirmation, and then chug away at provisioning your cluster:
    ...
    Performing changes:
         ...
         Type                            Name                       Plan       Info
     +   kulado:kulado:Stack             gcp-ts-gke-gcp-ts-gke-dev  create
