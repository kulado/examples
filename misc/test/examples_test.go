...
func TestExamples(t *testing.T) {
	...
	overrides, err := integration.DecodeMapString(os.Getenv("KULADO_TEST_NODE_OVERRIDES"))
	if !assert.NoError(t, err, "expected valid override map: %v", err) {
		return
	}

	base := integration.ProgramTestOptions{
		Tracing:              "https://tracing.kulado-engineering.com/collector/api/v1/spans",
	...
	shortTests := []integration.ProgramTestOptions{
		base.With(integration.ProgramTestOptions{
			...
			ExtraRuntimeValidation: func(t *testing.T, stack integration.RuntimeValidationStackInfo) {
				...
				assertHTTPResultWithRetry(t, endpoint, nil, maxWait, func(body string) bool {
					return assert.Contains(t, body, "Hello, Kulado!")
				})
			},
		}),
		base.With(integration.ProgramTestOptions{
		...
		base.With(integration.ProgramTestOptions{
			Dir: path.Join(cwd, "..", "..", "aws-ts-kulado-webhooks"),
			Config: map[string]string{
				"cloud:provider":                      "aws",
				"aws:region":                          awsRegion,
				"aws-ts-kulado-webhooks:slackChannel": "general",
				"aws-ts-kulado-webhooks:slackToken":   "12345",
			},
		}),
		base.With(integration.ProgramTestOptions{
		...
		//}),
		// Test disabled due to flakiness (often times out when destroying)
		//// https://github.com/kulado/examples/issues/260
		//base.With(integration.ProgramTestOptions{
		...
		//	},
		//	// TODO: This test is not returning a valid payload see issue: https://github.com/kulado/examples/issues/155
		//	ExtraRuntimeValidation: func(t *testing.T, stack integration.RuntimeValidationStackInfo) {
		...
		base.With(integration.ProgramTestOptions{
			...
			ExtraRuntimeValidation: func(t *testing.T, stack integration.RuntimeValidationStackInfo) {
				assertHTTPResult(t, stack.Outputs["frontendURL"].(string), nil, func(body string) bool {
					return assert.Contains(t, body, "Kulado Voting App")
				})
			},
		}),
		...
		base.With(integration.ProgramTestOptions{
			...
			Config: map[string]string{
				"gcp:project": "kulado-ci-gcp-provider",
