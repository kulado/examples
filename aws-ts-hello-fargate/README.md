...
To do this, we use Kulado infrastructure as code to provision an
...
- [Connect Kulado with your AWS account](https://kulado.io/quickstart/aws/setup.html) (if your AWS CLI is
...
2. Set your desired AWS region:
    ...
    ```bash
    $ kulado config set aws:region us-east-1 # any valid AWS region will work
    ```

3. Deploy everything with a single `kulado up` command. This will show you a preview of changes first, which
   includes all of the required AWS resources (clusters, services, and the like). Don't worry if it's more than
   you expected -- this is one of the benefits of Kulado, it configures everything so that so you don't need to!
    ...
    Updating (dev):
         ...
         Type                                                        Name                        Status
     +   kulado:kulado:Stack                                         aws-ts-hello-fargate-dev    created
    ...
    Permalink: https://app.kulado.com/acmecorp/aws-ts-hello-fargate/dev/updates/1
    ```

4. At this point, your app is running! The URL was published so it's easy to interact with:

    ```bash
    $ curl http://$(kulado stack output url)
    <h3>Hello World!</h3>
