...
The Kulado program constructs the S3 bucket, and constructs an `aws.s3.BucketObject` object
for every file in `config.pathToWebsiteContents`. This is essentially tracks every file on
your static website as a Kulado-managed resource. So a subsequent `kulado up` where the
...
const contentFile = new aws.s3.BucketObject(
    relativeFilePath,
    {
        ...
        source: new kulado.asset.FileAsset(filePath),
    });
```

The Kulado program then creates an `aws.cloudfront.Distribution` resource, which will serve
...
This error will occurr when the state of the ETag get out of sync between the Kulado Service
and AWS. (Which can happen when inspecting the CloudFront distribution in the AWS console.)

This will get fixed in Kulado soon, but for the time being you can find workaround steps in
the [issue on GitHub](kulado/kulado/issues/1449).
...
It may be more efficient to not manage individual files using Kulado and and instead just use the
AWS CLI to sync local files with the S3 bucket directly.

Remove the call to `crawlDirectory` and run `kulado up`. Kulado will then delete the contents
of the S3 bucket, and no longer manage their contents. Then do a bulk upload outside of Kulado
using the AWS CLI.
