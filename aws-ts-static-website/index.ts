...
// Load the Kulado program configuration. These act as the "parameters" to the Kulado program,
// so that different Kulado Stacks can be brought up using the same code.

const stackConfig = new kulado.Config("static-website");
const config = {
...
let certificateArn: kulado.Input<string> = config.certificateArn!;
...
// Export properties from this stack. This prints them at the end of `kulado up` and
// makes them easier to access from the kulado.com.
export const contentBucketUri = kulado.interpolate `s3://${contentBucket.bucket}`;
