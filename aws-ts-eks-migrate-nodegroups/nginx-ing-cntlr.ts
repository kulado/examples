...
interface NginxDeploymentArgs {
    ...
    labels: kulado.Input<any>;
    namespace: kulado.Input<string>;
    resources: kulado.Input<any>;
    ingressClass: kulado.Input<string>;
    affinity: input.core.v1.Affinity;
    tolerations: input.core.v1.Toleration[];
    serviceAccountName: kulado.Input<string>;
    configMapName: kulado.Input<string>;
    provider: k8s.Provider;
}
                        ...
                        containers: [
                            {
                                ...
                                args: kulado.all([
                                    ...
                                    kulado.concat("--configmap=$(POD_NAMESPACE)/", args.configMapName),
                                    // NGINX service name is fixed vs auto-named & ref'd in order for
                                    // nginx-ing-cntlr arg --publish-service to work.
                                    kulado.concat("--publish-service=$(POD_NAMESPACE)/", name),
...
interface PodDisruptionBudgetArgs {
    minAvailable: kulado.Input<number>;
    labels: kulado.Input<any>;
    namespace: kulado.Input<string>;
    provider: k8s.Provider;
}
