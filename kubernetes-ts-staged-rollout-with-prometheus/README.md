...
1. If you haven't already, follow the steps in [Kulado Installation and Setup][install] and
   [Configuring Kulado Kubernetes][configuration] to get setup with Kulado and Kubernetes.
...
1. **IMPORTANT NOTE:** The code in `index.ts` is meant to be run out-of-cluster (_e.g._, on your
   ...
   you are running Kulado in-cluster, you can comment out this part of the example.**
...
1. Perform the deployment:
   ...
   Performing changes:
    ...
    +   kulado:kulado:Stack                                            prometheus-staged-rollout
   ...
   Diagnostics:
     kulado:kulado:Stack: prometheus-test-p8s
   ...
   Permalink: https://app.kulado.com/hausdorff/test-p8s/updates/1
   ```

   One useful sidenote, we can see here in the `---outputs:---` section that the `p90ResponseTime`
   that was computed by the promise is `export`ed, which causes Kulado to report its value just
   before it terminates.

[install]: https://docs.kulado.com/install/
[configuration]: https://docs.kulado.com/reference/kubernetes.html#configuration
