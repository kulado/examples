...
import { meta } from "@kulado/kubernetes/types/input";
...
//
// This is useful for when we can't run Kulado in-cluster, in which case simply calling to the
// appropriate KubeDNS URL is not sufficient.
export function forwardPrometheusService(
    service: kulado.Input<k8s.core.v1.Service>,
    deployment: kulado.Input<k8s.extensions.v1beta1.Deployment>,
    opts: PromPortForwardOpts
): kulado.Output<() => void> {
    if (kulado.runtime.isDryRun()) {
        return kulado.output(() => {});
    }

    return kulado.all([service, deployment]).apply(([s, d]) => kulado.all([s.metadata, d.urn])).apply(([meta]) => {
...
export interface CheckLatencyOpts {
    ...
    forwarderHandle: kulado.Output<() => void>;
}
...
export function checkHttpLatency(
...
): kulado.Output<string> {
    if (kulado.runtime.isDryRun()) {
        return kulado.output(Promise.resolve("<computed value>"));
    }
    ...
    return kulado.all([canary.urn, kill]).apply(([_, kill]) => {
