...
1. Configure GCP Auth
    ...
    * Login using `gcloud`
    ...
    > configure your service account](https://kulado.io/install/gcp.html). The
...
1. Create a new Kulado stack, which is an isolated deployment target for this example:
...
1. Set the required GCP configuration variables:
    ...
    ```bash
    $ kulado config set gcp:project <YOUR_GCP_PROJECT_HERE>
    $ kulado config set gcp:zone us-west1-a     // any valid GCP Zone here
    ```
...
1. Stand up the GKE cluster:
    ...
    Running `kulado up` will deploy the GKE cluster. Note, provisioning a
	...
	Previewing update (gke-demo):
		...
		Type                            Name                             Plan
	+   kulado:kulado:Stack             gcp-ts-gke-hello-world-gke-demo  create
	+   ├─ gcp:container:Cluster        helloworld                       create
	+   ├─ kulado:providers:kubernetes  helloworld                       create
	...
	Updating (gke-demo):
		...
		Type                            Name                             Status
	+   kulado:kulado:Stack             gcp-ts-gke-hello-world-gke-demo  created
	+   ├─ gcp:container:Cluster        helloworld                       created
	+   ├─ kulado:providers:kubernetes  helloworld                       created
...
1. Access the Kubernetes Cluster using `kubectl`
    ...
    Of course, by doing so, resources are outside of Kulado's purview, but this simply
    demonstrates that all the `kubectl` commands you're used to will work.

1. Experimentation

    From here on, feel free to experiment. Simply making edits and running `kulado up` afterwords, will incrementally update your stack.
    ...
    const guestbook = new k8s.yaml.ConfigFile("guestbook",
        {
            file: "https://raw.githubusercontent.com/kulado/kulado-kubernetes/master/tests/examples/yaml-guestbook/yaml/guestbook.yaml",
            transformations: [
                (obj: any) => {
