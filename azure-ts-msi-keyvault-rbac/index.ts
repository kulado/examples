...
const connectionString = kulado.interpolate`Server=tcp:${sqlServer.name}.database.windows.net;Database=${database.name};`;
...
const blob = new azure.storage.ZipBlob("zip", {
    ...
    content: new kulado.asset.FileArchive("./webapp/bin/Debug/netcoreapp2.2/publish")
});

const clientConfig = kulado.output(azure.core.getClientConfig({}));
...
const vault = new azure.keyvault.KeyVault("vault", {
    ...
    accessPolicies: [{
        ...
        // secrets to/from the Key Vault. Otherwise, 'kulado up' and 'kulado destroy' operations will fail.
...
const secretUri = kulado.interpolate`${secret.vaultUri}secrets/${secret.name}/${secret.version}`;
...
const app = new azure.appservice.AppService("app", {
    ...
    appSettings: {
        ...
        "WEBSITE_RUN_FROM_ZIP": kulado.interpolate`@Microsoft.KeyVault(SecretUri=${secretUri})`,
...
// Work around a preview issue https://github.com/kulado/kulado-azure/issues/192
...
const blobPermission = new azure.role.Assignment("readblob", {
    principalId,
    scope: kulado.interpolate`${storageAccount.id}/blobServices/default/containers/${storageContainer.name}`,
...
export const endpoint = kulado.interpolate `https://${app.defaultSiteHostname}`;
