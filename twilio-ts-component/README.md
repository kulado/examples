...
1. Configure Twilio settings 
    ...
    ```
    $ kulado config set twilio:accountSid <your account sid from https://www.twilio.com/console>
    $ kulado config set --secret twilio:authToken <your auth token from https://www.twilio.com/console>
    $ kulado config set phoneNumberSid <the phone number sid from https://www.twilio.com/console/phone-numbers/>
    ```

1. Restore NPM modules via `npm install`.

1. Preview and run the deployment via `kulado up`. 
    ```
    $ kulado up
    ...
    +   kulado:kulado:Stack                  aws-serverless-twilio-component-twilio-dev  created
...
This example builds and uses a custom `kulado.CustomResource` to make it easy to spin up a SMS handler on Twilio. It could be extended to support Voice as well, by adding an additional handler to `twilio.IncomingPhoneNumberArgs`.

The custom resource itself is in [`twilio.ts`](./twilio.ts) and handles the work to use `@kulado/aws-serverless` to create a REST endpoint with `serverless.apigateway.API`. The handler registered with API Gateway does some of the teadious work of decoding the incoming event data and the delegates to the actual handler provided to the custom resource. 
