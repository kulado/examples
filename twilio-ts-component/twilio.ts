...
import * as serverless from "@kulado/aws-serverless";
import * as url from "url";
import { Callback } from "@kulado/aws-serverless/function";
import { APIArgs } from "@kulado/aws-serverless/api";

const config = new kulado.Config("twilio");
const accountSid = config.require("accountSid");
const authToken = config.require("authToken");

export class IncomingPhoneNumber extends kulado.ComponentResource {
    public /*out*/ readonly smsUrl: kulado.Output<string>;

    constructor(name: string, args: IncomingPhoneNumberArgs, opts? : kulado.ResourceOptions) {
        ...
        this.smsUrl = kulado.interpolate `${api.url}sms`;
