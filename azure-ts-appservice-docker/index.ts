...
import * as docker from "@kulado/docker";
...
export const helloEndpoint = kulado.interpolate`http://${helloApp.name}.azurewebsites.net/hello`;
...
const myImage = new docker.Image(customImage, {
    imageName: kulado.interpolate`${registry.loginServer}/${customImage}:v1.0.0`,
    build: {
        context: `./${customImage}`,
    },
    registry: {
...
const getStartedApp = new azure.appservice.AppService("get-started", {
    ...
    appSettings: {
      WEBSITES_ENABLE_APP_SERVICE_STORAGE: "false",
      DOCKER_REGISTRY_SERVER_URL: kulado.interpolate`https://${registry.loginServer}`,
    ...
    siteConfig: {
        alwaysOn: true,
        linuxFxVersion: kulado.interpolate`DOCKER|${myImage.imageName}`,
    },
    httpsOnly: true,
});

export const getStartedEndpoint = kulado.interpolate`http://${getStartedApp.name}.azurewebsites.net`;
