...
# A simple Slackbot running in AWS using Kulado.
...
The example contains a few useful patterns that show how to build a good Slackbot while taking advantage of a lot of conveniences that Kulado and the `aws` and `awsx` packages provide.
...
1.  Run `kulado up` to preview and deploy changes:
    ...
    Updating (mentionbot):
         ...
         Type                                Name                          Status 
     +   kulado:kulado:Stack                 aws-ts-slack-mentionbot       created
