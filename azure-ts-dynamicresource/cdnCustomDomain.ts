...
export interface CustomDomainOptions {
    ...
    profileName: kulado.Input<string>;
    endpointName: kulado.Input<string>;
    customDomainHostName: kulado.Input<string>;
    httpsEnabled: kulado.Input<boolean>;
}

/**
 * DynamicProviderInputs represents the inputs that are passed as inputs
 * to each function in the implementation of a `kulado.dynamic.ResourceProvider`.
...
class CDNCustomDomainResourceProvider implements kulado.dynamic.ResourceProvider {
    private name: string;
    ...
    private async getCDNManagementClient(): Promise<cdnManagement.CdnManagementClient> {
        ...
        if (!clientID || !clientSecret || !tenantID || !subscriptionID) {
            await kulado.log.info("Checking env vars for ARM credentials.", undefined, undefined, true);
        ...
        if (!clientID || !clientSecret || !tenantID || !subscriptionID) {
            await kulado.log.info("Env vars did not contain ARM credentials. Trying Az CLI.", undefined, undefined, true);
            ...
            await kulado.log.info("Using credentials from Az CLI.", undefined, undefined, true);
        } else {
    ...
    async check(olds: DynamicProviderInputs, news: DynamicProviderInputs): Promise<kulado.dynamic.CheckResult> {
        ...
        const failures: kulado.dynamic.CheckFailure[] = [];
    ...
    async diff(id: string, previousOutput: DynamicProviderOutputs, news: DynamicProviderInputs): Promise<kulado.dynamic.DiffResult> {
        ...
        if (previousOutput.customDomainHostName !== news.customDomainHostName || previousOutput.name !== this.name) {
            await kulado.log.warn("Changing the domain name properties will cause downtime.", undefined, undefined, true);
    ...
    async create(inputs: DynamicProviderInputs): Promise<kulado.dynamic.CreateResult> {
        ...
        if (inputs.httpsEnabled) {
            await kulado.log.info("Enabling HTTPS for the custom domain", undefined, undefined, true);
    ...
    async read(id: string, props: DynamicProviderOutputs): Promise<kulado.dynamic.ReadResult> {
        const cdnClient = await this.getCDNManagementClient();
     ...
     */
    async update(id: string, currentOutputs: DynamicProviderOutputs, newInputs: DynamicProviderInputs): Promise<kulado.dynamic.UpdateResult> {
        const cdnClient = await this.getCDNManagementClient();
 ...
 */
export class CDNCustomDomainResource extends kulado.dynamic.Resource {
    /**
    ...
    public readonly endpointName: kulado.Output<string>;
    public readonly customDomainHostName: kulado.Output<string>;
    public readonly httpsEnabled: kulado.Output<boolean>;

    // The following are properties set by the CDN rest client.
    public readonly customHttpsProvisioningState: kulado.Output<cdnManagement.CdnManagementModels.CustomHttpsProvisioningState>;
    public readonly customHttpsProvisioningSubstate: kulado.Output<cdnManagement.CdnManagementModels.CustomHttpsProvisioningSubstate>;
    public readonly provisioningState: kulado.Output<string>;
    public readonly resourceState: kulado.Output<cdnManagement.CdnManagementModels.CustomDomainResourceState>;
    public readonly type: kulado.Output<string>;

    constructor(name: string, args: CustomDomainOptions, opts?: kulado.CustomResourceOptions) {
